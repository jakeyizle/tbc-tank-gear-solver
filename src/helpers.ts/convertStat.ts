...
type Conversion = {
  toPercentageOrSkill: (value: number) => number;
...
const statConversions: Record<StatRatingName, Conversion> = {
  ...
  Defense: {
    toPercentageOrSkill: (v) => Math.floor(v / 2.3654),
    toRating: (v) => v * 2.3654,
  },
  Resilience: {
    toPercentageOrSkill: (v) => Math.floor(v / 39.4231),
    toRating: (v) => v * 39.4231,
  },
  Armor: {
    toPercentageOrSkill: (v) => v / (v + 10557.5),
    toRating: () => {throw new Error("Armor cannot be converted to rating")},
  },
  SpellCrit: {
  ...
  Agility: {
    ...
    toRating: () => {throw new Error("Cannot caluclate agility from dodge")}
  }
};

export const convertStatToPercentageOrSkill = (stat: StatModifier & { statName: StatRatingName }) => statConversions[stat.statName].toPercentageOrSkill(stat.value);
export const convertStatToRating = (stat: StatModifier & { statName: Exclude<StatRatingName, "Armor"> }) => statConversions[stat.statName].toRating(stat.value);
